...
public:
        ...
        , mBatchSize(batchSize)
    {
        ...
        {
            auto dims = context ? context->getTensorShape(context->getEngine().getIOTensorName(i)) : mEngine->getTensorShape(mEngine->getIOTensorName(i));
            std::cout << dims << std::endl;
            size_t vol = context || !mBatchSize ? 1 : static_cast<size_t>(mBatchSize);
            nvinfer1::DataType type = mEngine->getTensorDataType(mEngine->getIOTensorName(i));
            int vecDim = mEngine->getTensorVectorizedDim(mEngine->getIOTensorName(i));
            if (-1 != vecDim) // i.e., 0 != lgScalarsPerVector
            {
                int scalarsPerVec = mEngine->getTensorComponentsPerElement(mEngine->getIOTensorName(i));
...
private:
    ...
    void memcpyBuffers(const bool copyInput, const bool deviceToHost, const bool async, const cudaStream_t& stream = 0)
    {
        for (int i = 0; i < mEngine->getNbIOTensors(); i++)
        {
            ...
            const nvinfer1::TensorIOMode& mode = mEngine->getTensorIOMode(mEngine->getIOTensorName(i));
            if ((copyInput && mode == nvinfer1::TensorIOMode::kINPUT) || 
                (!copyInput && mode == nvinfer1::TensorIOMode::kOUTPUT))
            {
                if (async)
